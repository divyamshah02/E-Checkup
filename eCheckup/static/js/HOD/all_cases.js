...
function updateAllStats(allCases) {
...
function applyFilters() {
  ...
  if (selectedDateFilter !== "all") {
    ...
    switch (selectedDateFilter) {
      ...
      case "this-fy":
        // FY in India: April 1st – March 31st
        if (now.getMonth() + 1 < 4) {
          // before April → current FY started last year
          start = new Date(now.getFullYear() - 1, 3, 1)
          end = new Date(now.getFullYear(), 2, 31, 23, 59, 59)
        } else {
          start = new Date(now.getFullYear(), 3, 1)
          end = new Date(now.getFullYear() + 1, 2, 31, 23, 59, 59)
        }
        break

      case "last-fy":
        if (now.getMonth() + 1 < 4) {
          // before April → last FY = year-2 to year-1
          start = new Date(now.getFullYear() - 2, 3, 1)
          end = new Date(now.getFullYear() - 1, 2, 31, 23, 59, 59)
        } else {
          start = new Date(now.getFullYear() - 1, 3, 1)
          end = new Date(now.getFullYear(), 2, 31, 23, 59, 59)
...
function addEventListeners() {
  ...
  document.querySelectorAll(".date-filter").forEach((el) => {
    el.addEventListener("click", function (e) {
      e.preventDefault()
      selectedDateFilter = this.dataset.date
      renderTable()
    })
  })
